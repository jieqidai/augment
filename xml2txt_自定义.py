...
def convert_annotation(image_add):
    ...
    in_file = open('D:/Code/Vehicle-Car-detection-and-multilabel-classification-master/yolov3/data/coco/plate_xml/' + image_add + '.xml')
    
    out_file = open('D:/Code/Vehicle-Car-detection-and-multilabel-classification-master/yolov3/data/coco/plate_ctpntxt/%s.txt'%(image_add), 'w')
    ...
    for obj in root.iter('object'):
        ...
        bb = convert((w,h), b)
        #out_file.writelines(str(image_add)+'.jpg,' + str(cls_id) + "," +str(cls) + "," + ",".join([str(a) for a in bb]) + '\n')
        #out_file.writelines(str(image_add)+'.jpg,' + ",".join([str(a) for a in bb]) +'\n')
        out_file.writelines(",".join([str(a) for a in bb]) +'\n')


if not os.path.exists('D:/Code/Vehicle-Car-detection-and-multilabel-classification-master/yolov3/data/coco/plate_ctpntxt/'):
    os.makedirs('D:/Code/Vehicle-Car-detection-and-multilabel-classification-master/yolov3/data/coco/plate_ctpntxt/')
image_adds = open("D:/Code/Vehicle-Car-detection-and-multilabel-classification-master/yolov3/data/coco/plate_path.txt")
